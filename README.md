...
- Be a part of the community and help resolve [Issues](./issues)
...
If using the latest docker version and/or disabling selinux does not fix the issue then please file a issue request on the [issues](./issues) page.
...
```bash
wget ./master/docker-compose.yml
```
...
To illustrate linking with a postgresql container, we will use the [postgres](https://hub.docker.com/_/postgresql) image. When using postgresql image in production you should mount a volume for the postgresql data store. 
...
To illustrate linking with a mysql container, we will use the [mysql](https://hub.docker.com/_/mysql) image. When using docker-mysql in production you should mount a volume for the mysql data store. 
...
To illustrate linking with a redis container, we will use the [redis](https://hub.docker.com/_/redis) image. 

First, lets pull the redis image from the docker index.

```bash
docker pull redis:4.0.9-1
```
...
Copy the `ca.crt` file into the certs directory on the [datastore](#data-store). The `ca.crt` file should contain the root certificates of all the servers you want to trust. With respect to GitLab CI, this will be the contents of the gitlab_ci.crt file as described in the README
...
| `NGINX_HSTS_ENABLED` | Advanced configuration option for turning off the HSTS configuration. Applicable only when SSL is in use. Defaults to `true`. for use case scenario. |
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.3 app:rake gitlab:import:repos
```
...
>
> If you're using `postgresql` then please upgrade to `postgresql:9.4-18` or later and add `DB_EXTENSION=pg_trgm` to the environment of the PostgreSQL container (see: master/docker-compose.yml#L8).
...
> **Note**
>
> Upgrading to `11.7.3` from `gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `gitlab:8.0.5-1` before upgrading to `gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] gitlab:11.7.3
```
