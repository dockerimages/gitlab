...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gitlab:11.7.0 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `11.7.0` from `gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `gitlab:8.0.5-1` before upgrading to `gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -d [OPTIONS] gitlab:11.7.0
```
