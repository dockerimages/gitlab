...
This guide assumes that you are currently using `gitlab` and `gitlab-ci` for setting up your GitLab and CI requirements.

> **Note:**
>
> If your CI server and your GitLab server use the same database adapter no special care is needed. If your CI server uses MySQL and your GitLab server uses PostgreSQL you need to pass a special option in **Step 4 - Upgrade CI > Create CI backup**. If your CI server uses PostgreSQL and your GitLab server uses MySQL you cannot migrate your CI data to GitLab `8.0`, Please refer to https://github.com/docker-gitlab/issues/429#issuecomment-152799995 for instructions to migrate from MySQL to PostgreSQL first.
...
### Upgrade to `gitlab:7.14.3`

```bash
docker run -it --rm [OPTIONS] \
  gitlab:7.14.3 app:init
```

### Upgrade to `gitlab-ci:7.14.3-1`

```bash
docker run -it --rm [OPTIONS] \
  gitlab-ci:7.14.3-1 app:init
```
...
```bash
docker run -it --rm [OPTIONS] \
  gitlab:7.14.3 app:rake gitlab:backup:create
```
...
```bash
docker run -it --rm [OPTIONS] \
  gitlab-ci:7.14.3-1 app:rake backup:create
```
...
### Upgrade to `gitlab-ci:8.0.5`

```bash
docker run -it --rm [OPTIONS] \
  gitlab-ci:8.0.5 app:init
```
...
```bash
docker run -it --rm [OPTIONS] \
  gitlab-ci:8.0.5 app:rake backup:create
```
...
We are done with CI. If the rest of the migration goes was planned you will not need to start `gitlab-ci` ever again.

## Step 5 - Upgrade GitLab

Before we can upgrade to `gitlab:8.0.5-1`, we need to assign the value of `GITLAB_CI_SECRETS_DB_KEY_BASE` (from CI) to `GITLAB_SECRETS_DB_KEY_BASE` in GitLab's environment.
...
docker run -it --rm [OPTIONS] \
  --env GITLAB_CI_HOST=ci.example.com --env GITLAB_SECRETS_DB_KEY_BASE=xxxxxx \
  gitlab:8.0.5-1 app:rake ci:migrate
```
